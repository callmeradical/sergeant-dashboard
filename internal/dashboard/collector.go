	...
	for _, task := range tasks {
		...
		for _, project := range projects {
			...
			if worker.supervisorPane != "" {
				worker.stateDir = filepath.Join(c.FleetRoot, relative)
			}
			if !detail {
				...
				c.classifyHealth(ctx, filepath.Join(c.FleetRoot, relative), &worker, now, staleAfter,
					func() (string, time.Time, error) {
						return readRootScalarWithTime(root, filepath.Join(relativeCopy, "pane"))
					},
					func() FileMetadata { return rootFileMetadata(root, filepath.Join(relativeCopy, "worker.log")) },
				)
			}
...
func (c Collector) classifyHealth(ctx context.Context, stateDir string, worker *Worker, now time.Time, staleAfter time.Duration, readPane func() (string, time.Time, error), readActivity func() FileMetadata) {
	...
	if paneErr != nil || pane == "" {
		// No supervisor pane recorded: fall back to status + timestamp classification.
		activityAt := latestTime(worker.UpdatedAt, recentActivity.UpdatedAt)
	...
	inspectCtx, cancel := context.WithTimeout(context.WithoutCancel(ctx), supervisorProbeTimeout)
	live, err := inspect(inspectCtx, pane, stateDir)
	cancel()
	if err != nil {
...
// ageHealth returns "stale" when activityAt is further in the past than staleAfter,
// and "orphaned" otherwise. Used when supervisor pane inspection fails or is unavailable.
...
// inspectWorkers concurrently verifies supervisor pane liveness for workers
// that recorded a pane in their fleet state directory, then overrides their
// health accordingly. Workers without a supervisorPane are not touched.
...
func collectWorkerWithReaders(task, project string, now time.Time, staleAfter time.Duration, detail bool, readScalar func(string) (string, time.Time, error), readOperational func(string) FileMetadata) (Worker, []string) {
	...
	activityAt := latestTime(worker.UpdatedAt, latestTime(worker.Log.UpdatedAt, latestTime(worker.Message.UpdatedAt, worker.Diagnostic.UpdatedAt)))
	pane, _, _ := readScalar("pane")
	if pane != "" {
		// Defer live/dead classification to the concurrent inspectWorkers pass.
		worker.Health = ageHealth(activityAt, now, staleAfter)
