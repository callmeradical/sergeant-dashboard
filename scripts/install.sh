...
if [ "$(uname -s)" != "Linux" ]; then
	printf '%s\n' "sergeant-dashboard systemd installation supports Linux only" >&2
	exit 1
fi
...
install -m 0644 "$repo_dir/deploy/sergeant-dashboard-tailnet-proxy.service" "$unit_dir/sergeant-dashboard-tailnet-proxy.service"
systemctl --user daemon-reload
systemctl --user enable --now sergeant-dashboard.service
systemctl --user enable --now sergeant-dashboard-tailnet-proxy.service
